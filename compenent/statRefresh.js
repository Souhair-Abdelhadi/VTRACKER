...
hanldeLogOut = () => {
 ...
 getStat = () => {
    ...
    var getColor;
                  ...
                  tx.executeSql('SELECT * FROM usertablecondition1;',[],(tx,results) =>{
                    ...
                    this.setState({ color : conditionColor});
                     ...
                     if (conditionColor == "red" ){                      
                      db.transaction((tx)=>{
                      tx.executeSql('select count(*) as allpersons from persons1;',[],(tx,res)=>{
                        var allpersons = res.rows.item(0).allpersons;
                         if (allpersons > 0 ) {
                             tx.executeSql('delete from persons1 where mac in (select mac from persons1);',[],(tx,res)=>{
                                 console.log(' user is red and all contact are deleted');
  
                                 tx.executeSql('SELECT * FROM persons1 ;',[],(tx,results)=>{
               
                                  var len = results.rows.length;
                                  this.setState({ alertMessage : ' You are positive to virus, please go to hospital or contact the authority .',
                                  alertColor : 'red', personNombres : len,
                                });                               
                              });
                             ...
                             console.log('user is infected there no need to insert data');
                             tx.executeSql('SELECT * FROM persons1 ;',[],(tx,results)=>{
                    ...
                    db.transaction((tx) => {
                      tx.executeSql('SELECT * FROM persons1 ;',[],(tx,results)=>{
                          ...
                          var len = results.rows.length;
                          this.setState({ personNombres : len});
                          var dataList = results.rows.raw();
                          if (len > 0){
                              firebase.database()
                              .ref('/users/' + firebase.auth().currentUser.uid + '/' )
      ...
      db.transaction((tx) => {
        tx.executeSql('SELECT * FROM usertablecondition1;',[],(tx,results) =>{
          ...
          if (conditionColor == "red"){
            this.setState({ alertMessage : ' You are positive to virus, please go to hospital or contact the authority .',
                              alertColor : 'red',color : 'red',
                            });  
          }
          else if (conditionColor == "yellow"){
            this.setState({ alertMessage : 'Possibly you have the virus, please go to hospital for check.',
            alertColor : 'orange',color : 'yellow',
          });

          }
          else {
            this.setState({ alertMessage : '',
            alertColor : 'green',color : 'green',
          });
          }

          tx.executeSql('SELECT * FROM persons1;',[],(tx,results) => {              
            this.setState({ personNombres : results.rows.length });
            console.log(this.state.personNombres);
    ...
    //       tx.executeSql('SELECT count(*) as nombres FROM persons1  ;',[],(tx,results) => {              
    //         this.setState({ personNombres : results.rows.item(0).nombres });
    //         console.log(this.state.personNombres);
